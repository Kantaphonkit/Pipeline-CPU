...
#   -Worktree   snapshot rtl/*.v and asm/smoke.hex from the live working tree
...
if ($Worktree) {
    ...
    Copy-Item -Path "asm\smoke.hex" -Destination "$Shadow\asm\smoke.hex" -Force
    if (-not $?) {
        Pop-Location
        Write-Error "FAIL: failed to copy asm\smoke.hex from working tree"
        exit 1
    }
} else {
    Write-Host "== snapshotting rtl/ and asm/smoke.hex from 'git archive HEAD' (committed state only) =="
    ...
    git archive --format=tar -o $asmTar HEAD -- asm/smoke.hex
    if ($LASTEXITCODE -ne 0) {
        Pop-Location
        Write-Error "FAIL: git archive HEAD -- asm/smoke.hex failed"
        exit 1
    }
    tar -xf $asmTar -C $Shadow
    if ($LASTEXITCODE -ne 0) {
        Pop-Location
        Write-Error "FAIL: tar extract of asm/smoke.hex snapshot failed"
        exit 1
    }
...
foreach ($f in @("synth.log", "synth.jou", "utilization.txt", "timing.txt", "clocks.txt")) {
...
foreach ($f in @("synth.log", "synth.jou", "utilization.txt", "timing.txt", "clocks.txt")) {
    ...
    if (Test-Path $src) {
        Copy-Item -Path $src -Destination (Join-Path $reportsDir $f) -Force
    }
}
...
Write-Host "PASS: synthesis complete. Reports in $reportsDir"
exit 0
